...
const { sendAttendanceEmail } = require('../config/emailService');
                ...
                if(data.confidence >= 0.5){
                    try {
                        const emailResult = await sendAttendanceEmail({
                            ...
                            studentName: data.name || student.name,
                        ...
                        console.error(`Email sending failed for ${student.email}:`, emailError.message);
                        emailResults.push({
                            success: false,
                            email: student.email,
